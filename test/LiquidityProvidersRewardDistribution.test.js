...
contract('LiquidityProvidersRewardDistribution', accounts => {
  ...
  describe('distribute', () => {
    ...
    it('should distribute', async () => {
      ...
      const receipt = await distributionContract.distribute(snapshotBlockNumber, liquidityProviders, rewards, { from: distributor });
      expectEvent(receipt, 'Distributed', { snapshotBlockNumber });
      expect(receipt.receipt.gasUsed).to.be.lte(8000000);
      ...
      expect(await stakeToken.balanceOf(distributionContract.address)).to.be.bignumber.equal(contractBalance.sub(total));
    });
    it('fails if not a distributor', async () => {
      const contractBalance = ether('1000');
      await stakeToken.mint(distributionContract.address, contractBalance);
      await expectRevert(
        distributionContract.distribute(snapshotBlockNumber, liquidityProviders, rewards, { from: accounts[2] }),
    ...
    it('fails if the sum of rewards is greater than contract balance', async () => {
      ...
      await stakeToken.mint(distributionContract.address, contractBalance);
      await expectRevert(
        distributionContract.distribute(snapshotBlockNumber, liquidityProviders, rewards, { from: distributor }),
